...
class sfWidgetFormSchema extends sfWidgetForm implements ArrayAccess
{
  ...
  public function render($name, $values = array(), $attributes = array(), $errors = array())
  {
    ...
    $hiddenRows = array();
    $errorRows = array();
	$parent_name= str_replace(array("[","]"),"_",$name);
    ...
    foreach ($this->positions as $name)
    {
	  ...
	  if($widget instanceof sfWidgetFormSelect ){
		  if(!isset($widgetAttributes["class"])){
			$widgetAttributes["class"] = " form-select"; 
		  }else{
			$widgetAttributes["class"] .= " form-select";  
		  }
	  }else
	  if($widget instanceof sfWidgetFormChoice and count($widget->getOption("choices"))> 0  and $widget->getOption("expanded")){
		  if(!isset($widgetAttributes["class"])){
			$widgetAttributes["class"] = " radio-inline"; 
		  }else{
			$widgetAttributes["class"] .= " radio-inline";  
		  }
	  }else{
      ...
      else
      {
        ...
        $rows[] = $formFormat->formatRow($label, $field, $error, $this->getHelp($name), null, $parent_name.$name);
      }
    }
